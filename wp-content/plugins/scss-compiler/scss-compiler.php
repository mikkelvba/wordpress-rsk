<?php
/**
 * Plugin Name: Scss Compiler
 * Plugin URI: http://brandbyhand.dk
 * Description: Plugin to compile scss files on wordpress websites.
 * Version: 1.0
 * Author: Mikkel Veldt Andersen
 * Author URI: http://mikkelva.dk
 * License: GPLv3
 */
...
include_once WPSCSS_PLUGIN_DIR . '/class/class-scss-compiler.php'; // Compiling Manager
...
if( $scss_dir_setting == false || $css_dir_setting == false ) {
  function wpscss_settings_error() {
      echo '<div class="error">
        <p><strong>Scss-compiler</strong> requires both directories be specified. <a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=wpscss_options">Please update your settings.</a></p>
      </div>';
  }
...
// Checks if directory exists
  ...
  function wpscss_settings_error() {
      echo '<div class="error">
        <p><strong>Scss-compiler:</strong> One or more specified directories does not exist. Please create the directories or <a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=wpscss_options">update your settings.</a></p>
      </div>';
  }
  add_action('admin_notices', 'wpscss_settings_error');
  return 0; //exits
